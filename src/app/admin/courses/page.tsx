...
import {
  Plus, Pencil, Trash2, Save, X, Check, Upload, Loader2,
  ImageIcon, Video, ArrowLeft, Eye, EyeOff, GripVertical,
  HelpCircle,
} from "lucide-react";
...
export default function AdminCourses() {
  ...
  const persistCourse = async (proceedTo?: Step) => {
    ...
    const slug = editingCourse.title.toLowerCase().trim().replace(/\s+/g, "-").replace(/[^a-z0-9\u0980-\u09FF-]/g, "");
  ...
  const closeBuilder = () => { setBuilderOpen(false); setStep(1); setSavedCourseId(null); setEditingCourse(null); setModules([]); };
  ...
  const openEdit = (course: any) => { setEditingCourse(course); setIsNew(false); setSavedCourseId(course.id); setStep(1); setBuilderOpen(true); };
  const openCurriculum = async (course: any) => { setEditingCourse(course); setIsNew(false); setSavedCourseId(course.id); await loadModules(course.id); setStep(2); setBuilderOpen(true); };
  ...
  const saveTopicDraft = async () => {
    ...
    if (error) { toast.error(error.message); return; }
    setTopicDraft(null); loadModules(id);
  };
  ...
  const saveQuizDraft = async () => {
    if (!quizDraft || !quizDraft.title.trim()) { toast.error("কুইজের নাম দিন"); return; }
              ...
              <CardContent className="flex items-center justify-between p-4">
                <div className="flex items-center gap-4">
                  <div className="h-12 w-20 overflow-hidden rounded bg-muted">
                    {course.thumbnail_url && <Image src={course.thumbnail_url} alt="" width={80} height={48} className="h-full w-full object-cover" />}
                  </div>
                  <div>
      ...
      <div className="sticky top-0 z-10 border-b bg-background">
        <div className="flex items-center justify-between gap-4 px-4 py-3 md:px-6">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button size="sm" onClick={async () => { setEditingCourse((p: any) => ({ ...p, is_published: true })); await persistCourse(); toast.success("পাবলিশড"); }}>পাবলিশ</Button>
          </div>
        </div>
        <div className="flex items-center gap-3 border-t px-4 py-2 md:hidden">
          <StepBtn n={1} label="Basics" /><StepBtn n={2} label="Curriculum" /><StepBtn n={3} label="Additional" />
        </div>
      </div>
          ...
          <div className="space-y-6">
                ...
                <Card>
                  ...
                  <CardContent>
                    <Tabs defaultValue="general">
                      <TabsList><TabsTrigger value="general">General</TabsTrigger><TabsTrigger value="instructor">Instructor</TabsTrigger></TabsList>
                      <TabsContent value="general" className="space-y-4 pt-4">
                        <div className="grid gap-3 sm:grid-cols-2">
                          <div><Label className="mb-1.5 block">সময়কাল</Label><Input placeholder="40 Hours / 2 Month" value={editingCourse.duration_text ?? ""} onChange={(e) => setEditingCourse({ ...editingCourse, duration_text: e.target.value })} /></div>
                          <div><Label className="mb-1.5 block">এনরোলমেন্ট সংখ্যা</Label><Input type="number" value={editingCourse.enrollment_count ?? 0} onChange={(e) => setEditingCourse({ ...editingCourse, enrollment_count: Number(e.target.value) })} /></div>
                        </div>
                      </TabsContent>
                      <TabsContent value="instructor" className="space-y-4 pt-4">
                        <div><Label className="mb-1.5 block">ইন্সট্রাক্টরের নাম</Label><Input value={editingCourse.instructor_name ?? ""} onChange={(e) => setEditingCourse({ ...editingCourse, instructor_name: e.target.value })} /></div>
                        <div><Label className="mb-1.5 block">ইন্সট্রাক্টর সম্পর্কে</Label><Textarea rows={3} value={editingCourse.instructor_bio ?? ""} onChange={(e) => setEditingCourse({ ...editingCourse, instructor_bio: e.target.value })} /></div>
                      </TabsContent>
                    </Tabs>
                  </CardContent>
                </Card>
            ...
            {step === 2 && (
              <div className="space-y-4">
                ...
                {modules.map((mod) => (
                  <div key={mod.id} className="rounded-lg border-2 border-primary/30 bg-card p-4">
                    {editingTopicId === mod.id ? (
                      <div className="space-y-3">
                        <div className="flex items-start gap-2">
                          ...
                          <Input value={topicEdit.title} onChange={(e) => setTopicEdit({ ...topicEdit, title: e.target.value })} className="bg-muted/40" />
                        </div>
                        <Textarea rows={3} value={topicEdit.summary} onChange={(e) => setTopicEdit({ ...topicEdit, summary: e.target.value })} className="ml-6 bg-muted/40" />
                        <div className="flex justify-end gap-3"><Button variant="ghost" size="sm" onClick={() => setEditingTopicId(null)}>Cancel</Button><Button size="sm" onClick={() => updateTopic(mod.id)}>Ok</Button></div>
                      </div>
                    ) : (
                      <div className="space-y-2">
                        <div className="flex items-start justify-between gap-2">
                          <div className="flex flex-1 items-start gap-2">
                            ...
                            <div className="flex-1"><h3 className="font-semibold">{mod.title}</h3>{mod.summary && <p className="mt-1 text-sm text-muted-foreground">{mod.summary}</p>}</div>
                          </div>
                    ...
                    {lessonDraft && lessonDraft.moduleId === mod.id && (
                      <div className="ml-6 mt-3 space-y-2 rounded border bg-muted/20 p-3">
                        ...
                        <div className="flex justify-end gap-2"><Button variant="ghost" size="sm" onClick={() => setLessonDraft(null)}>Cancel</Button><Button size="sm" onClick={saveLessonDraft}>Ok</Button></div>
                      </div>
                    )}
                    {quizDraft && quizDraft.moduleId === mod.id && (
                      <div className="ml-6 mt-3 space-y-2 rounded border bg-muted/20 p-3">
                        ...
                        <div className="flex justify-end gap-2"><Button variant="ghost" size="sm" onClick={() => setQuizDraft(null)}>Cancel</Button><Button size="sm" onClick={saveQuizDraft}>Ok</Button></div>
                      </div>
                    )}
                    <div className="mt-3 flex gap-2">
                      <Button variant="outline" size="sm" onClick={() => { setLessonDraft({ moduleId: mod.id, title: "", videoUrl: "", duration: "" }); setQuizDraft(null); }}><Plus className="mr-1 h-3.5 w-3.5" />Lesson</Button>
                      <Button variant="outline" size="sm" onClick={() => { setQuizDraft({ moduleId: mod.id, title: "" }); setLessonDraft(null); }}><Plus className="mr-1 h-3.5 w-3.5" />Quiz</Button>
                    </div>
                  </div>
                ))}
                {topicDraft && (
                  <div className="space-y-3 rounded-lg border-2 border-primary/40 bg-card p-4">
                    ...
                    <div className="flex justify-end gap-3"><Button variant="ghost" size="sm" onClick={() => setTopicDraft(null)}>Cancel</Button><Button size="sm" onClick={saveTopicDraft}>Ok</Button></div>
                  </div>
                )}
                {!topicDraft && (
                ...
                {modules.length === 0 && !topicDraft && <p className="py-6 text-center text-muted-foreground">কোনো টপিক নেই।</p>}
              </div>
            )}

            {step === 3 && (
              <Card>
                <CardHeader className="py-4"><CardTitle className="text-base">Overview</CardTitle></CardHeader>
                <CardContent className="space-y-5">
                  <div><Label className="mb-1.5 block">What Will I Learn?</Label><Textarea rows={4} value={editingCourse.what_will_learn ?? ""} onChange={(e) => setEditingCourse({ ...editingCourse, what_will_learn: e.target.value })} /></div>
                  <div><Label className="mb-1.5 block">Target Audience</Label><Textarea rows={3} value={editingCourse.target_audience ?? ""} onChange={(e) => setEditingCourse({ ...editingCourse, target_audience: e.target.value })} /></div>
                  <div>
                    <Label className="mb-1.5 block">Total Course Duration</Label>
                    <div className="grid grid-cols-2 gap-3">
                      <div className="relative"><Input type="number" min={0} value={editingCourse.duration_hours ?? 0} onChange={(e) => setEditingCourse({ ...editingCourse, duration_hours: parseInt(e.target.value) || 0 })} className="pr-20" /><span className="absolute right-2 top-1/2 -translate-y-1/2 rounded border bg-muted px-2 py-0.5 text-xs text-muted-foreground">hour(s)</span></div>
                      <div className="relative"><Input type="number" min={0} max={59} value={editingCourse.duration_minutes ?? 0} onChange={(e) => setEditingCourse({ ...editingCourse, duration_minutes: parseInt(e.target.value) || 0 })} className="pr-20" /><span className="absolute right-2 top-1/2 -translate-y-1/2 rounded border bg-muted px-2 py-0.5 text-xs text-muted-foreground">min(s)</span></div>
                    </div>
                  </div>
                  <div><Label className="mb-1.5 block">Materials Included</Label><Textarea rows={4} value={editingCourse.materials_included ?? ""} onChange={(e) => setEditingCourse({ ...editingCourse, materials_included: e.target.value })} /></div>
                  <div><Label className="mb-1.5 block">Requirements</Label><Textarea rows={4} value={editingCourse.requirements ?? ""} onChange={(e) => setEditingCourse({ ...editingCourse, requirements: e.target.value })} /></div>
                  <div className="border-t pt-5">
                    <h3 className="mb-3 text-base font-semibold">🏆 সার্টিফিকেট সেটিংস</h3>
                    <label className="mb-3 flex cursor-pointer items-center gap-2 text-sm">
                      <Checkbox checked={editingCourse.certificate_enabled ?? true} onCheckedChange={(v) => setEditingCourse({ ...editingCourse, certificate_enabled: !!v })} />
                      <span>এই কোর্সের জন্য সার্টিফিকেট সক্রিয় করুন</span>
                    </label>
                    <div className="space-y-3">
                      <div><Label className="mb-1.5 block">সার্টিফিকেট শিরোনাম</Label><Input value={editingCourse.certificate_title ?? ""} onChange={(e) => setEditingCourse({ ...editingCourse, certificate_title: e.target.value })} /></div>
                      <div><Label className="mb-1.5 block">সার্টিফিকেট বডি টেক্সট</Label><Textarea rows={3} value={editingCourse.certificate_body ?? ""} onChange={(e) => setEditingCourse({ ...editingCourse, certificate_body: e.target.value })} /></div>
                      <div><Label className="mb-1.5 block">স্বাক্ষর (ঐচ্ছিক)</Label><Input value={editingCourse.certificate_signature ?? ""} onChange={(e) => setEditingCourse({ ...editingCourse, certificate_signature: e.target.value })} /></div>
                    </div>
                  </div>
                </CardContent>
              </Card>
            ...
            <Card>
              <CardContent className="p-4">
                ...
                <Select value={editingCourse.is_published ? "public" : "draft"} onValueChange={(v) => setEditingCourse({ ...editingCourse, is_published: v === "public" })}>
                  <SelectTrigger><SelectValue>{editingCourse.is_published ? <div className="flex items-center gap-2"><Eye className="h-4 w-4" />Public</div> : <div className="flex items-center gap-2"><EyeOff className="h-4 w-4" />Draft</div>}</SelectValue></SelectTrigger>
                  <SelectContent>
              ...
              <CardContent className="p-4">
                ...
                {editingCourse.thumbnail_url ? (
                  <div className="relative">
                    <Image src={editingCourse.thumbnail_url} alt="" width={320} height={144} className="h-36 w-full rounded object-cover" />
              ...
              <CardContent className="p-4">
                ...
                {editingCourse.intro_video_url ? (
                  ...
                  <div className="flex flex-col items-center justify-center gap-2 rounded-md border-2 border-dashed bg-muted/30 px-3 py-6">
                    ...
                    <Button size="sm" variant="outline" onClick={() => { const url = prompt("YouTube/Vimeo URL দিন:"); if (url) setEditingCourse({ ...editingCourse, intro_video_url: url }); }}><Plus className="mr-1 h-3 w-3" />Add from URL</Button>
                  </div>
                )}
              </CardContent>
            </Card>
            ...
            <Card>
              <CardContent className="p-4">
                ...
                <div className="mb-3 flex gap-4">
                  <label className="flex cursor-pointer items-center gap-2 text-sm"><input type="radio" checked={Number(editingCourse.price) === 0} onChange={() => setEditingCourse({ ...editingCourse, price: 0, original_price: null })} />Free</label>
                  <label className="flex cursor-pointer items-center gap-2 text-sm"><input type="radio" checked={Number(editingCourse.price) > 0} onChange={() => setEditingCourse({ ...editingCourse, price: editingCourse.price > 0 ? editingCourse.price : 500 })} />Paid</label>
                </div>
                {Number(editingCourse.price) > 0 && (
                  <div className="space-y-2">
                    <div><Label className="text-xs">মূল্য (৳)</Label><Input type="number" value={editingCourse.price} onChange={(e) => setEditingCourse({ ...editingCourse, price: Number(e.target.value) })} /></div>
                    <div><Label className="text-xs">পূর্বের মূল্য</Label><Input type="number" value={editingCourse.original_price ?? ""} onChange={(e) => setEditingCourse({ ...editingCourse, original_price: e.target.value ? Number(e.target.value) : null })} /></div>
                  </div>
                )}
              </CardContent>
            </Card>
