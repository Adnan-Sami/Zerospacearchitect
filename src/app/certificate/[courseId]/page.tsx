  ...
  useEffect(() => {
    supabase.auth.getSession().then(async ({ data: { session } }) => {
      ...
      const { data: course } = await supabase
        .from("courses")
        .select(
          "title, certificate_enabled, certificate_title, certificate_body, certificate_signature"
        )
        .eq("id", courseId)
        .single();
      if (course && (course as any).certificate_enabled === false) {
        toast.error("এই কোর্সের জন্য সার্টিফিকেট উপলব্ধ নয়");
        router.push("/my-courses");
        return;
      }
      ...
      if (allLessons.length === 0) {
        toast.error("কোর্সে কোনো লেসন নেই");
        router.push("/my-courses");
        return;
      }
      ...
      setData({
        ...
        issuedDate: new Date(cert?.issued_at || Date.now()).toLocaleDateString(
          "bn-BD"
        ),
