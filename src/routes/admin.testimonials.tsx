...
function AdminTestimonials() {
  ...
  const save = async () => {
    if (!editing.name || !editing.comment) { toast.error("নাম ও মন্তব্য দিন"); return; }
    const payload = {
      name: editing.name,
      role: editing.role ?? "",
  ...
  return (
    <div>
      <div className="mb-6 flex items-center justify-between">
        ...
        <Button onClick={() => setEditing({ is_active: true, rating: 5, sort_order: 0 })}><Plus className="mr-1 h-4 w-4" />নতুন</Button>
      </div>
