...
function AdminBanners() {
  ...
  const save = async () => {
    if (!editing.image_url) { toast.error("ছবির URL দিন"); return; }
    const payload = {
      title: editing.title ?? "",
  ...
  return (
    <div>
      <div className="mb-6 flex items-center justify-between">
        ...
        <Button onClick={() => setEditing({ is_active: true, sort_order: 0 })}><Plus className="mr-1 h-4 w-4" />নতুন ব্যানার</Button>
      </div>

      {editing && (
        <Card className="mb-6">
          <CardHeader><CardTitle>{editing.id ? "এডিট" : "নতুন"} ব্যানার</CardTitle></CardHeader>
          <CardContent className="space-y-3">
            <div><Label>শিরোনাম (ঐচ্ছিক)</Label><Input value={editing.title ?? ""} onChange={(e) => setEditing({ ...editing, title: e.target.value })} /></div>
            <div><Label>সাব-টাইটেল (ঐচ্ছিক)</Label><Input value={editing.subtitle ?? ""} onChange={(e) => setEditing({ ...editing, subtitle: e.target.value })} /></div>
            <div>
            ...
            <div><Label>অথবা ছবির URL</Label><Input value={editing.image_url ?? ""} onChange={(e) => setEditing({ ...editing, image_url: e.target.value })} /></div>
      ...
      <div className="space-y-3">
        ...
        {items.length === 0 && <p className="text-center text-muted-foreground">কোনো ব্যানার নেই।</p>}
      </div>
    </div>
  );
}
