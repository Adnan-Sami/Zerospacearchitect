...
function AdminSettings() {
  ...
  const persist = async (next: any) => {
    ...
    }).eq("id", next.id).select().maybeSingle();
  ...
  const save = async () => {
    ...
    const ok = await persist({ ...settings, site_name: settings.site_name.trim() });
  ...
  if (!settings) return <p className="text-muted-foreground">লোড হচ্ছে...</p>;
    ...
    <div className="max-w-2xl">
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <div>
            <Label>লোগো</Label>
            {settings.logo_url && <img src={settings.logo_url} alt="logo" className="my-2 h-16 rounded border bg-muted p-2" />}
            <div className="flex gap-2">
              <Input value={settings.logo_url} onChange={(e) => setSettings({ ...settings, logo_url: e.target.value })} placeholder="লোগো URL" />
              <label className="inline-flex">
                <input type="file" accept="image/*" className="hidden" onChange={(e) => e.target.files?.[0] && uploadLogo(e.target.files[0])} />
                <Button asChild variant="outline" disabled={uploading}><span><Upload className="mr-1 h-4 w-4" />{uploading ? "..." : "আপলোড"}</span></Button>
              </label>
            </div>
          </div>
          <div>
          ...
          <Button onClick={save}><Save className="mr-1 h-4 w-4" />সেভ করুন</Button>
        </CardContent>
      </Card>
    </div>
  );
