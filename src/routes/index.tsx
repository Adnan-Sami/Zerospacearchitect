...
import { ArrowRight, BookOpen, Code, Palette, TrendingUp, Briefcase, Award, Star, Clock, ChevronRight, Phone } from "lucide-react";
...
import heroInstructors from "@/assets/hero-instructors.jpg";
import instructorCta from "@/assets/instructor-cta.png";

export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      { title: "শিক্ষা - বাংলাদেশের সেরা অনলাইন লার্নিং প্ল্যাটফর্ম" },
      { name: "description", content: "ঘরে বসে শিখুন সেরা ইন্সট্রাক্টরদের কাছ থেকে। কোর্স কিনুন, ভিডিও দেখুন, সার্টিফিকেট পান।" },
    ],
  }),
  component: HomePage,
});
...
function HomePage() {
  ...
  const heroImage = useSiteContent("home.hero.image") || heroInstructors;
  ...
  const stat1v = useSiteContent("home.stats.1.value"); const stat1l = useSiteContent("home.stats.1.label");
  const stat2v = useSiteContent("home.stats.2.value"); const stat2l = useSiteContent("home.stats.2.label");
  const stat3v = useSiteContent("home.stats.3.value"); const stat3l = useSiteContent("home.stats.3.label");
  const whyTitle = useSiteContent("home.why.title");
  const whySubtitle = useSiteContent("home.why.subtitle");
  const why1t = useSiteContent("home.why.1.title"); const why1d = useSiteContent("home.why.1.desc");
  const why2t = useSiteContent("home.why.2.title"); const why2d = useSiteContent("home.why.2.desc");
  const why3t = useSiteContent("home.why.3.title"); const why3d = useSiteContent("home.why.3.desc");
  ...
  const instImage = useSiteContent("home.instructor.image") || instructorCta;
  ...
  useEffect(() => {
    Promise.all([
      supabase.from("courses").select("*, categories(name)").eq("is_published", true).order("created_at", { ascending: false }).limit(8),
      supabase.from("courses").select("*").eq("is_published", true).order("enrollment_count", { ascending: false }).limit(6),
    ...
    <div className="flex min-h-screen flex-col">
        ...
        <div className="mx-auto max-w-7xl rounded-3xl border border-sky-200 bg-gradient-to-br from-sky-50 via-background to-blue-50 px-6 py-10 shadow-sm md:px-12 md:py-14">
          <div className="grid items-center gap-10 md:grid-cols-2">
            <div className="relative z-10">
              ...
              <Link to="/courses">
                <Button size="lg" className="rounded-full bg-sky-500 px-8 text-base font-semibold shadow-lg hover:bg-sky-600">
                  {heroCta}
                  <ArrowRight className="ml-2 h-4 w-4" />
                </Button>
              </Link>

              <svg className="absolute -bottom-6 left-56 hidden h-20 w-48 text-sky-300 md:block" viewBox="0 0 200 80" fill="none" aria-hidden="true">
                <path d="M5 15 Q 80 -10, 130 30 T 190 60" stroke="currentColor" strokeWidth="2" strokeDasharray="4 6" strokeLinecap="round" fill="none" />
                <path d="M180 52 L 192 60 L 182 70" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" fill="none" />
              </svg>
            </div>

            <div className="relative">
              <div className="relative overflow-hidden rounded-2xl shadow-2xl ring-1 ring-sky-100">
                <img src={heroImage} alt="ইন্ডাস্ট্রি এক্সপার্টদের সাথে শিখুন" width={1024} height={768} className="h-full w-full object-cover" />
            ...
            {banners.map((b) => {
              const inner = (
                <div className="group overflow-hidden rounded-2xl shadow-md transition-all hover:shadow-xl">
                  <img src={b.image_url} alt={b.title ?? ""} loading="lazy" className="h-32 w-full object-cover transition-transform group-hover:scale-105 md:h-36" />
                </div>
              );
              return b.link_url ? (
        ...
        <section className="bg-sky-50/50 px-4 py-14 md:px-8">
          <div className="mx-auto max-w-7xl">
            <div className="mb-10 text-center">
              <h2 className="mb-3 text-3xl font-extrabold text-foreground md:text-4xl">{latestTitle}</h2>
              <p className="text-sm text-muted-foreground md:text-base">{latestSubtitle}</p>
            </div>

            <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {courses.slice(0, 3).map((c) => (
                <div key={c.id} className="overflow-hidden rounded-2xl border border-sky-100 bg-card shadow-sm transition-shadow hover:shadow-xl">
                  <div className="relative">
                    {c.thumbnail_url ? (
                      <img src={c.thumbnail_url} alt={c.title} className="h-52 w-full object-cover" />
                    ) : (
                      <div className="flex h-52 w-full items-center justify-center bg-muted text-muted-foreground">কোর্স</div>
                    )}
                  ...
                  <div className="p-5">
                    ...
                    <div className="flex items-center justify-between border-t pt-4">
                      ...
                      <Link to="/courses/$courseId" params={{ courseId: c.id }}>
                        <Button variant="outline" className="rounded-full border-sky-400 text-sky-600 hover:bg-sky-50 hover:text-sky-700">
                          কোর্সটি কিনুন
                        </Button>
                      </Link>
                    </div>
            ...
            <div className="mt-10 flex justify-center">
              <Link to="/courses">
                <Button size="lg" className="rounded-full bg-gradient-to-r from-sky-500 to-blue-500 px-10 text-base font-semibold shadow-lg hover:from-sky-600 hover:to-blue-600">
      ...
      {bestsellers.length > 0 && (
        <section className="px-4 py-14 md:px-8">
          <div className="mx-auto max-w-7xl">
            <div className="mb-10 text-center">
              <h2 className="mb-3 text-3xl font-extrabold text-foreground md:text-4xl">{bestsellerTitle}</h2>
              <p className="text-sm text-muted-foreground md:text-base">{bestsellerSubtitle}</p>
            </div>
      ...
      <section className="px-4 py-12 md:px-8">
        <div className="mx-auto grid max-w-6xl gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {[
            { icon: "🎓", value: stat1v, label: stat1l, gradient: "from-sky-400 to-sky-500" },
            { icon: "👁️", value: stat2v, label: stat2l, gradient: "from-blue-400 to-blue-500" },
            { icon: "👨‍🏫", value: stat3v, label: stat3l, gradient: "from-blue-500 to-indigo-600" },
          ].map((s) => (
            <div key={s.label} className={`flex items-center gap-5 rounded-2xl bg-gradient-to-r ${s.gradient} px-6 py-6 shadow-lg`}>
              <div className="flex h-20 w-20 shrink-0 items-center justify-center rounded-full bg-white text-4xl shadow-md">
                {s.icon}
              </div>
              <div className="text-white">
                <p className="text-3xl font-extrabold leading-tight md:text-4xl">{s.value}</p>
      ...
      <section className="bg-sky-50/40 px-4 py-14 md:px-8">
        <div className="mx-auto max-w-6xl">
          <div className="mb-10 text-center">
            <h2 className="mb-3 text-3xl font-extrabold text-foreground md:text-4xl">{whyTitle}</h2>
          ...
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {[
              ...
              <div key={f.title} className="rounded-2xl bg-card px-6 py-10 text-center shadow-sm transition-shadow hover:shadow-xl">
      ...
      <section className="px-4 py-10 md:px-8">
        <div className="mx-auto flex max-w-7xl flex-col items-start justify-between gap-6 rounded-2xl bg-black px-8 py-10 shadow-xl md:flex-row md:items-center md:px-12">
          <div>
            <h3 className="mb-2 text-xl font-bold text-white md:text-2xl">{callTitle}</h3>
            <p className="text-sm text-gray-300">{callSubtitle}</p>
          </div>
          <a href={`tel:${callPhone.replace(/[^+\d]/g, "")}`} className="inline-flex items-center gap-2 rounded-md bg-green-500 px-6 py-3 text-base font-semibold text-white shadow-lg transition-colors hover:bg-green-600">
        ...
        <div className="relative mx-auto max-w-7xl overflow-hidden rounded-2xl bg-blue-600 shadow-xl">
          <div className="grid items-center gap-6 md:grid-cols-2">
            <div className="px-8 py-12 md:px-14 md:py-16">
              <h3 className="mb-4 text-xl font-bold leading-relaxed text-white md:text-2xl">{instTitle}</h3>
            ...
            <div className="relative flex h-full items-end justify-center md:justify-end">
              <img src={instImage} alt="ইন্সট্রাকটর" loading="lazy" width={768} height={768} className="h-72 w-auto object-contain md:h-80" />
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="px-4 py-10 md:px-8">
        <div className="mx-auto max-w-7xl rounded-2xl bg-blue-600 px-6 py-10 shadow-xl md:px-12">
          ...
          >
            <h3 className="text-center text-lg font-bold text-white md:text-left md:text-xl">{newsletterTitle}</h3>
            <div className="flex w-full max-w-xl items-center rounded-full bg-white p-1.5 shadow-md">
              ...
              />
              <Button type="submit" className="rounded-full bg-blue-600 px-6 font-semibold hover:bg-blue-700">
                {newsletterCta}
              </Button>
            </div>
          </form>
      ...
      {testimonials.length > 0 && (
        <section className="px-4 py-12">
          <div className="mx-auto max-w-7xl">
            <h2 className="mb-8 text-center text-2xl font-bold text-foreground">{testimonialsTitle}</h2>
