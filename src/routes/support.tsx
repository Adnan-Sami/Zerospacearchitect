...
export const Route = createFileRoute("/support")({
  head: () => ({
    meta: [
      { title: "সাপোর্ট - ZeroSpace Architect" },
      { name: "description", content: "যেকোনো প্রয়োজনে ZeroSpace এর সাথে যোগাযোগ করুন।" },
    ],
  }),
              ...
              <div className="rounded-xl bg-white p-5 shadow-sm">
                ...
                <ul className="space-y-2 text-sky-600">
                  <li className="flex items-center gap-2">
                    ...
                    <a href={`tel:${phoneNum.replace(/[^+\d]/g, "")}`} className="hover:underline">{phoneNum}</a>
                  </li>
                </ul>
              </div>
              <div className="rounded-xl bg-white p-5 shadow-sm">
                ...
                <ul className="space-y-2 text-sky-600">
                  <li className="flex items-center gap-2">
                    ...
                    <a href={`mailto:${email1}`} className="hover:underline">{email1}</a>
                  </li>
                  <li className="flex items-center gap-2">
                    <Mail className="h-4 w-4" />
                    <a href={`mailto:${email2}`} className="hover:underline">{email2}</a>
                  </li>
                </ul>
              </div>
            </div>
      ...
      <section className="py-12">
        <div className="mx-auto grid max-w-7xl gap-6 px-4 md:grid-cols-2 md:px-8">
          <div className="rounded-2xl bg-sky-50 p-6">
            <h3 className="mb-4 text-center font-bold text-gray-800">{tut1Title}</h3>
            <div className="aspect-video overflow-hidden rounded-lg bg-black">
              <iframe src={tut1Video} title={tut1Title} className="h-full w-full" allowFullScreen />
            </div>
          </div>
          <div className="rounded-2xl bg-sky-50 p-6">
            <h3 className="mb-4 text-center font-bold text-gray-800">{tut2Title}</h3>
            <div className="aspect-video overflow-hidden rounded-lg bg-black">
              <iframe src={tut2Video} title={tut2Title} className="h-full w-full" allowFullScreen />
            </div>
          </div>
        </div>
      </section>
