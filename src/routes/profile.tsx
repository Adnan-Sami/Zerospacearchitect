...
export const Route = createFileRoute("/profile")({
  head: () => ({ meta: [{ title: "প্রোফাইল - শিক্ষা" }] }),
...
function ProfilePage() {
  ...
  useEffect(() => {
    supabase.auth.getSession().then(({ data: { session } }) => {
      ...
      supabase.from("profiles").select("*").eq("user_id", session.user.id).single().then(({ data }) => {
        if (data) { setFullName(data.full_name); setPhone(data.phone ?? ""); setAvatarUrl(data.avatar_url ?? ""); }
      });
    });
  }, [navigate]);
  ...
  const handleAvatarUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const { error: dbErr } = await supabase.from("profiles").update({ avatar_url: publicUrl }).eq("user_id", userId);
    if (dbErr) toast.error(dbErr.message);
    else { setAvatarUrl(publicUrl); toast.success("প্রোফাইল ছবি আপডেট হয়েছে!"); }
    setUploading(false);
  };
  ...
  const handlePasswordChange = async () => {
    setPwdMessage("");
    setPwdError("");
    if (newPassword.length < 6) { setPwdError("পাসওয়ার্ড কমপক্ষে ৬ অক্ষরের হতে হবে"); return; }
    if (newPassword !== confirmPassword) { setPwdError("পাসওয়ার্ড মিলছে না"); return; }
    setPwdSaving(true);
    ...
    <div className="flex min-h-screen flex-col">
      ...
      <div className="mx-auto w-full max-w-2xl flex-1 px-4 py-8">
        <div className="mb-6 flex items-center justify-between">
          ...
          <Link to="/dashboard"><Button variant="outline" size="sm"><LayoutDashboard className="mr-1 h-4 w-4" />ড্যাশবোর্ড</Button></Link>
        </div>
        ...
        <div className="space-y-6">
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="flex flex-col items-center gap-3 pb-2">
                ...
                <p className="text-xs text-muted-foreground">{uploading ? "আপলোড হচ্ছে..." : "ছবি পরিবর্তনের জন্য ক্যামেরা আইকনে ক্লিক করুন"}</p>
              </div>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              {pwdMessage && <p className="rounded-md bg-primary/10 p-3 text-sm text-primary">{pwdMessage}</p>}
              {pwdError && <p className="rounded-md bg-destructive/10 p-3 text-sm text-destructive">{pwdError}</p>}
              <div className="space-y-2">
