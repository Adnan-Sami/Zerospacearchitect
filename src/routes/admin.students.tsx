...
function AdminStudents() {
  ...
  return (
    <div>
      <h1 className="mb-4 text-2xl font-bold">শিক্ষার্থী তালিকা ({students.length})</h1>
      <Input placeholder="নাম বা ফোন দিয়ে খুঁজুন..." value={search} onChange={(e) => setSearch(e.target.value)} className="mb-4 max-w-md" />
          ...
          <Card key={s.id} className={s.is_banned ? "border-destructive/40 bg-destructive/5" : ""}>
            <CardContent className="flex items-center justify-between p-4">
              <div className="flex items-center gap-3">
                <div>
                  <h3 className="font-medium">{s.full_name || "নাম নেই"} {s.is_banned && <Badge variant="destructive" className="ml-2">ব্যানড</Badge>}</h3>
                  <p className="text-sm text-muted-foreground">{s.phone || "ফোন নেই"} · যোগদান: {new Date(s.created_at).toLocaleDateString("bn-BD")}</p>
                </div>
              </div>
              <Button size="sm" variant={s.is_banned ? "outline" : "destructive"} onClick={() => toggleBan(s)}>
