...
function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    const { data: profile } = await supabase.from("profiles").select("is_banned").eq("user_id", data.user.id).maybeSingle();
    ...
    navigate({ to: roleData ? "/admin" : "/dashboard" });
    setLoading(false);
  };
          ...
          <CardContent>
            <form onSubmit={handleLogin} className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <Input id="loginId" type="text" placeholder="০১XXXXXXXXX অথবা admin@example.com" value={loginId} onChange={(e) => setLoginId(e.target.value)} required />
              </div>
              <div className="space-y-2">
                <Label htmlFor="password">পাসওয়ার্ড</Label>
                <Input id="password" type="password" placeholder="পাসওয়ার্ড" value={password} onChange={(e) => setPassword(e.target.value)} required />
              </div>
              <Button type="submit" className="w-full" disabled={loading}>
                {loading ? "অপেক্ষা করুন..." : "লগ-ইন"}
              </Button>
              <p className="text-center text-sm text-muted-foreground">
                অ্যাকাউন্ট নেই? <Link to="/register" className="text-primary hover:underline">রেজিস্ট্রেশন করুন</Link>
              </p>
            </form>
          </CardContent>
        </Card>
